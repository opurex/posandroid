...
public class CompositionInstance extends Product {
    ...
    public CompositionInstance(Product product, Composition compo) {
        super(product.id, product.label, product.barcode, product.price,
                product.taxedPrice,
                product.taxId, product.scaled,
                product.hasImage, product.discountRate, product.discountRateEnabled, product.isPrepaid());
        this.compo = compo;
        this.components = new HashMap<Composition.Group, Product>();
    }

    /** Create a Product for the composition instance as regular product,
     * forgetting about it's content. */
    public Product extractMainProduct() {
        return new Product(id, label, barcode, price, taxedPrice, taxId, scaled, hasImage, discountRate, discountRateEnabled, isPrepaid());
    }
