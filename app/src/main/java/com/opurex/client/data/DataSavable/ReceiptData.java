...
public class ReceiptData extends AbstractJsonDataSavable {
    ...
    /* Hacky way to convert CompositionInstances to Products because gson
     * cannot handle subclasses neatly. */
    @Override
    protected void save(List<Object> objs, com.opurex.client.utils.file.File file) {
        List<Receipt> toSave = new ArrayList<Receipt>();
        for (Receipt r : (List<Receipt>) objs.get(0)) {
            toSave.add(Receipt.convertFlatCompositions(r));
        }
        List<Object> result = new ArrayList<>();
        result.add(toSave);
        super.save(result, file);
    }
}
